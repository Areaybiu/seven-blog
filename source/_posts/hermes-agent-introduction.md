...
这意味着一个非常有趣的使用模式：**你可以在手机 Telegram 上给它发任务，它在云服务器上跑，任务完成后推送结果给你。**
...
用内置的 Cron 调度功能，设置定时任务：

```
每天早上 8 点搜索 AI 行业新闻，整理成简报发到我的 Telegram
```
...
在飞书/钉钉上配置接入后，直接在聊天框里发指令，它会搜索、整理、总结，然后回复。
...
- 至少需要一个 LLM 提供商的 API Key（OpenRouter、OpenAI、Anthropic 等）
...
**项目地址：** [GitHub - NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)
