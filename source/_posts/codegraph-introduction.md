...
categories:
  - 技术
cover: false
description: 当 AI 编程助手需要理解你的代码时，它在做什么？CodeGraph 用预建知识图谱替代了低效的文件扫描，让 AI 从"翻遍所有书架"变成"查目录"。本文深入解析其技术原理、架构设计、实际效果和真实用户反馈。
---
...
**知乎用户实测：**
> "同样的任务，CodeGraph 消耗 16,211 tokens，按 DeepSeek V4-Flash 计费标准（输入每百万 token 2 元），大概只花 0.03 元左右。"

**构建速度：**
> "构建、同步挺快的，比正常 LLM 推理快。" —— golemon

**大型项目效果：**
> "感觉大型项目可以，我现在中型项目（两百多代码文件的项目）感觉读一次会省一点 token。企业里面的大型项目可能更好。" —— golemon

### 5.2 需要注意的问题

**与其他工具配合：**
> "token 是省了，但结合其他 skill 效果不是很好，需要调教。其他工具会基于实际运行结果进行'判断'，有了 CodeGraph 其他工具会根据它构建的'图谱'进行总结判断，正确率会下降不少。" —— 冒蓝火的哒哒哒

**数据质疑：**
> "我其实很怀疑其中的一些数据。比如'等它读完十几个文件，几万 token 已经烧掉'，我觉得，这应该是一种推断，前提是编程智能体以加载整个文件的方式来读取内容。" —— 涵树

**对比实验的严谨性：**
> "目前无法仅凭正文信息确认相关数据的统计依据。这类工具的实际效果可能因项目规模、agent 配置和具体问题场景而有差异。" —— 知乎直答
